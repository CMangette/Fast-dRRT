...
function [V_last,Tree] = Expand(Tree,V_last,G,valid_indices,H,Q_goal)
    ...
    [~ , best_free_idx , ~ , lowest_free_cost] = Best_Expansion_Parent(Vnew,N,Tree,G);
...
function V_hybrid = Brute_Force_Connector(V1,V2,G)
    ...
    Xf = Get_State_Polygons(V2);
    
    for i = 1:(R-1)
	
        for j = (i+1):R
            local_priority = Priority_i_j(Xi{i},Xf{i},vehicle_paths{i},Xi{j},Xf{j},vehicle_paths{j});
...
function priority = Priority_i_j(Vi_1,Vi_2,path_i,Vj_1,Vj_2,path_j)
    ...
    Vi_2_and_path_j_overlap = overlaps(Vi_2,path_j);
    path_i_and_Vj_1_overlap = overlaps(path_i,Vj_1);
    path_i_and_Vj_2_overlap = overlaps(path_i,Vj_2);
    
    if Vi_1_and_path_j_overlap && not(Vi_2_and_path_j_overlap)
        priority = 1;
    elseif not(Vi_1_and_path_j_overlap) && Vi_2_and_path_j_overlap
        priority = -1;
    elseif Vi_1_and_path_j_overlap && Vi_2_and_path_j_overlap
        priority = 1;
    elseif path_i_and_Vj_1_overlap && not(path_i_and_Vj_2_overlap)
        priority = -1;
    elseif not(path_i_and_Vj_1_overlap) && path_i_and_Vj_2_overlap
        priority = 1;
    elseif path_i_and_Vj_1_overlap && path_i_and_Vj_2_overlap
        priority = 1;
    else
        priority = 2;
    end
